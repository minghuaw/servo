...
impl TextEncoderCommon {
    pub(crate) fn new_inherited() -> Self {
        ...
        let utf16_encoder = encoding_rs::UTF_16BE.new_decoder();
        #[cfg(not(target_endian = "big"))]
        let utf16_encoder = encoding_rs::UTF_16LE.new_decoder();
    ...
    pub(crate) fn encode_code_units(&self, code_units: &[u16], last: bool) -> Fallible<Vec<u8>> {
        let bytes: &[u8] = unsafe { std::mem::transmute(code_units) };
        let mut encoder = self.utf16_encoder.borrow_mut();
        let mut output = String::with_capacity(encoder.max_utf8_buffer_length(bytes.len())
            .ok_or_else(|| Error::Type("Expected utf-8 encoding would overflow".to_owned()))?);

        let (result, _read, _replaced) = encoder.decode_to_string(&bytes, &mut output, last);
